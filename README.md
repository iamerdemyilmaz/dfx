...
- **Quizzes.** Each question is a `<fieldset class="q">` with `data-answer`. Multiple choice uses radio inputs whose `value` matches `data-answer`. Numeric questions add `data-type="numeric"` and `data-tolerance`. The pass mark is 70 percent, set by `PASS_MARK` in `course.js`. Each module except the introduction also opens with a three-question "Before you start" form marked `data-pre`; it is graded in the browser with explanations but never recorded as progress. Module 0 has no quiz at all and is flagged `quiz: false` in `MODULES`, which excludes it from the progress count.
